    ...
    <div className="h-full flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <label className="text-sm font-medium text-foreground mb-3 block">Add photos (1-3)</label>
          <div className="flex gap-3">
            {[0, 1, 2].map((index) => (
              <div key={index} className="relative w-32 h-32 rounded-2xl overflow-hidden bg-secondary shadow-soft">
                {photos[index] ? (
                  <>
                  ...
                  >
                    ...
                    <span className="text-[10px] text-muted-foreground">{index === 0 ? "Add" : "Add"}</span>
                  </button>
                )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <label className="text-sm font-medium text-foreground mb-3 block">Travel vibes (select up to 4)</label>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 pb-8 bg-gradient-to-t from-background via-background to-transparent z-10">
        <Button variant="accent" size="lg" className="w-full" disabled={!isValid} onClick={handleContinue}>
          Continue
        </Button>
      </div>
